    ...
    *,
    ...
    try:
        ...
        if inspect.isawaitable(handler_result):
            # The synchronous Celery compatibility path cannot safely drive an
            # async repository on a running event loop.  Do not leave a coroutine
            # unclosed or mark the event successful without its durable write.
            close = getattr(handler_result, "close", None)
            if callable(close):
                close()
            resolved_deduplication.release(key)
            result = ProcessingResult.retryable(
                envelope.event_id,
                error_code="ASYNC_HANDLER_REQUIRED",
            )
        else:
...
async def process_envelope_async(
    envelope: EventEnvelope,
    *,
    registry: HandlerRegistry | None = None,
    deduplication: DeduplicationStore | None = None,
    ...
    """Async counterpart used by the production poller for DB-backed handlers."""
